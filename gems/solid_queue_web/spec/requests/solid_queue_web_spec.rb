...
RSpec.describe "SolidQueueWeb dashboard", type: :request do
  ...
  describe "queues" do
    ...
    it "forbids pausing when the action is disabled" do
      SolidQueueWeb.config.enable_pause = false
      post "/admin/solid_queue/queues/x/pause"
      expect(response).to have_http_status(:forbidden)
    ensure
      SolidQueueWeb.config.enable_pause = true
    end
  end
  ...
  describe "failed executions" do
    ...
    it "forbids retry when disabled" do
      failed = make_failed(create_job)
      SolidQueueWeb.config.enable_retry = false

      post "/admin/solid_queue/failed/#{failed.id}/retry"
      expect(response).to have_http_status(:forbidden)
    ensure
      SolidQueueWeb.config.enable_retry = true
    end
  end
